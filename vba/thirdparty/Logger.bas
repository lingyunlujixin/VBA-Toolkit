...
' udf
Private logCache As New Collection

Private udfFile As String
...
Private Sub writeFile(ByRef context As Collection)
      
    Dim objLogWriter As New FileOperation
         
    If udfFile = Empty Then
        udfFile = "d:\debug.log"
    End If
    
     ' write to file
     objLogWriter.OpenFile udfFile, "APPEND"
    
    For Each o_line In logCache
        objLogWriter.WriteLine (o_line)
    Next

    objLogWriter.CloseFile
    
    Set objLogWriter = Nothing

End Sub

Public Sub flush()

    If logCache.Count > 0 Then
        
        writeFile logCache
        
        ' clear and new
        Set logCache = New Collection
...
'
  ...
  logCache.Add " - " & Now & " - " & LevelValue & " - " & IIf(From <> "", From & ": ", "") & Message
...
Public Sub ImmediateLog(level As Long, Message As String, From As String)
  ...
  End Select
'
  Debug.Print " - " & Now & " - " & LevelValue & " - " & IIf(From <> "", From & ": ", "") & Message
